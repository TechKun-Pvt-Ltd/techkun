...
const _padding = property("_padding")`
	...
	initial-value: 0;
`;
const _width = property("_width")`
	syntax: "${widthSyntax}";
	inherits: true;
	initial-value: 0;
`;
const _height = property("_height")`
	syntax: "${widthSyntax}";
	inherits: true;
	initial-value: 0;
`;
...
const texts = [
	"Misalignment", "Lack of documentation", "Scope creep", "Technical debt", "Design debt", "Short-term optimization"
];
...
function GraphicWindow() {
	...
	useEffect(() => {
		const interval = setInterval(_ => setItemIndex(prev => (prev + 1) % texts.length), 3000);
		return () => clearInterval(interval);
	}, []);

	return <Window title="graphic">
		<div css={css`
			flex-grow: 1;
			padding: 32px 24px;
		`}>
			...
			`}>
				...
				<TextScramble as="span">{texts[itemIndex]}</TextScramble>
			</p>
		</div>
	</Window>;
}
	...
	return <section>
		...
		`}>
			...
			<div css={css`
                ...
                position: relative;
			`}>
				<div className="screen" css={css`
					${_borderRadius}: 36px;
					${_padding}: 8px;
                    position: relative;

					.window[data-title="graphic"] {
						${_height}: 640px;
					}
                    .window[data-title="chat"] {
						${_width}: min(100%, 448px);
						${_height}: max(560px, max-content);
                        position: absolute;
                        right: 48px;
                        bottom: -64px;
                    }
